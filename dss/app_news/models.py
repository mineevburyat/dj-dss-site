...
class ImageMedia(models.Model):
    ...
    def get_large_html(self):
        ...
        return mark_safe(f"<a href={self.large.url}>{name} ({width}x{height})</a>")
        
    def get_medium_html(self):
        img = PImage.open(self.medium)
        width, height = img.size
        name = self.medium.name
        return mark_safe(f"<a href={self.medium.url}>{name} ({width}x{height})</a>")
    
    def get_small_html(self):
        img = PImage.open(self.thumbnail)
        width, height = img.size
        name = self.thumbnail.name
        return mark_safe(f"<a href={self.thumbnail.url}>{name} ({width}x{height})</a>")
